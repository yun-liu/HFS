...
struct float4_
{
	_CPU_AND_GPU_CODE_ float4_(){}
	_CPU_AND_GPU_CODE_ float4_(float x_, float y_, float z_, float w_) {
		x = x_, y = y_, z = z_, w = w_;
	}
...
struct float2_
{
	_CPU_AND_GPU_CODE_ float2_(){}
	_CPU_AND_GPU_CODE_ float2_(float x_, float y_) {
		x = x_, y = y_;
	}
	volatile float x, y;
};

_CPU_AND_GPU_CODE_ inline float4_ operator+= (float4_ &a, float4_ b)
{
	a.x += b.x;
	a.y += b.y;
	a.z += b.z;
...
_CPU_AND_GPU_CODE_ inline float2_ operator+= (float2_ &a, float2_ b)
{
	a.x += b.x;
	a.y += b.y;
	return a;
