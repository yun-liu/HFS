...
void HFSSegment::trainStageI(int trainSample)
{
	...
	for (int n = 0; n < trainNum; n++) {
		...
		for (int i = 0; i < num_css; i++) {
			for (int j = 0; j < adjacent[i].size(); j++) {
                Mat row1 = hist.row(i), row2 = hist.row(adjacent[i][j]);
				float score = getChiHist(row1, row2);
...
void HFSSegment::runDataSet(float c, int min_size)
{
	...
	if (opendir(_S(_FResDir)) == NULL)
		CmFile::MkDir(_FResDir);
	if (opendir(_S(_FDisDir)) == NULL)
...
bool HFSSegment::matRead(CStr &filename, vecM &_M)
{
	...
	if (f == NULL)
		return false;
	char buf[8];
	if(fread(buf, sizeof(char), 5, f) == 0)
        std::cout << "Read CvMat data file " << _S(filename) << "error!" << endl;
	if (strncmp(buf, "CmMat", 5) != 0)	{
		std::cout << "Invalidate CvMat data file " << _S(filename) << endl;
		return false;
	}
	int matCnt;
	if(fread(&matCnt, sizeof(int), 1, f) == 0)
        std::cout << "Read CvMat data file " << _S(filename) << "error!" << endl;
	_M.resize(matCnt);
	for (int i = 0; i < matCnt; i++) {
		int headData[3]; // Width, Height, Type
		if(fread(headData, sizeof(int), 3, f) == 0)
            std::cout << "Read CvMat data file " << _S(filename) << "error!" << endl;
		Mat M(headData[1], headData[0], headData[2]);
		if(fread(M.data, sizeof(char), M.step * M.rows, f) == 0)
            std::cout << "Read CvMat data file " << _S(filename) << "error!" << endl;
		M.copyTo(_M[i]);
	}
	std::fclose(f);
