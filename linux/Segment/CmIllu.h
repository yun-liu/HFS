...
namespace CmIllu{
		...
		for (int r = 0; r < label.rows; r++){
			...
			for (int c = 0; c < label.cols; c++)
				if (c > 0 && color[c - 1] == color[c])
                    label.row(r).col(c) = label.row(r).col(c-1);
				else
					rgb2Label(color[c], label.row(r).col(c));
		}
	}
	...
	template<typename T> void label2Rgb(const Mat_<T> &label, Mat &color3u){
		...
		for (int r = 0; r < label.rows; r++){
			...
			for (int c = 0; c < label.cols; c++)
				if (c > 0 && label.row(r).col(c-1) == label.row(r).col(c))
					color[c] = color[c-1];
				else
					label2Rgb(label.row(r).col(c), color[c]);
		}
